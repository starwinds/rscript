...
ggcorplot = function(data,var_text_size,cor_text_limits,smooth_method='lm'){
  ...
  for(i in 1:length(data)){
    if(sd(data[,i])>0){data[,i]=(data[,i]-mean(data[,i]))/sd(data[,i])}
    else delete_column <- c(delete_column,i)
  }
  # sd 가 0 인경우, 즉 모든 값이 같은 column은 제외 
  if(is.null(delete_column)!=TRUE)data <- data[,-delete_column]
